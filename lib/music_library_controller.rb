class MusicLibraryController
    ...
    def initialize(path = "./db/mp3s")
      importer_instance = MusicImporter.new(path)
      importer_instance.import
    end

    def call 
        input = "" # needs to be defined before can loop
    ...
    def play_song
        ...
        if (1..Song.all.length).include?(input) # .. creates a range
          song = Song.all.sort{ |a, b| a.name <=> b.name }[input - 1] # when listed was sorted
        end
